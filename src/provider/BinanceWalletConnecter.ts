...
export type WalletConnectParameters = Evaluate<
  ...
  > &
    ExactPartial<
      Pick<EthereumProviderOptions, 'showQrModal'>
    >
>

BinanceWalletConnecter.type = 'walletConnect' as const
export function BinanceWalletConnecter(
  parameters: WalletConnectParameters
) {
  const isNewChainsStale =
    parameters.isNewChainsStale ?? true
  ...
  return createConnector<Provider, Properties, StorageItem>(
    (config) => ({
      id: 'walletConnect',
      name: 'WalletConnect',
      type: walletConnect.type,
      async setup() {
        const provider = await this.getProvider().catch(
          () => null
        )
        if (!provider) return
      ...
      async connect({ chainId, ...rest } = {}) {
        try {
          ...
          if (!targetChainId) {
            const state =
              (await config.storage?.getItem('state')) ?? {}
          ...
          const accounts = (await provider.enable()).map(
            (x) => getAddress(x)
          )
      ...
      async getProvider({ chainId } = {}) {
        async function initProvider() {
          const optionalChains = config.chains.map(
            (x) => x.id
          ) as [number]
          ...
          const rpc = config.chains.reduce(
            (rpcProps, chain) => ({
              ...rpcProps,
      ...
      async switchChain({ chainId }) {
        ...
        try {
          ...
          const namespaceChains =
            this.getNamespaceChainsIds()
          const namespaceMethods =
            this.getNamespaceMethods()
          const isChainApproved =
            namespaceChains.includes(chainId)

          if (
            !isChainApproved &&
            namespaceMethods.includes(
              'wallet_addEthereumChain'
            )
          ) {
            ...
            const requestedChains =
              await this.getRequestedChainsIds()
            this.setRequestedChainsIds([
              ...requestedChains,
              chainId,
            ])
      ...
      getNamespaceChainsIds() {
        if (!provider_) return []
        const chainIds = provider_.session?.namespaces[
          NAMESPACE
        ]?.chains?.map((chain) =>
      ...
      getNamespaceMethods() {
        if (!provider_) return []
        const methods = provider_.session?.namespaces[
          NAMESPACE
      ...
      async isChainsStale() {
        ...
        const connectorChains = config.chains.map(
          (x) => x.id
        )
        const namespaceChains = this.getNamespaceChainsIds()
        if (
          namespaceChains.length &&
          !namespaceChains.some((id) =>
