...
export function hasInjectedProvider({
  ...
  if (
    namespace &&
    typeof getWindowProviderNamespace(namespace) !==
      'undefined'
  )
    return true
  if (
    flag &&
    typeof getExplicitInjectedProvider(flag) !== 'undefined'
  )
    return true
  return false
}
...
function getInjectedProvider({
  ...
  if (namespace) {
    ...
    const windowProvider =
      getWindowProviderNamespace(namespace)
...
export function getInjectedConnector({
  ...
  const provider = target
    ? target
    : getInjectedProvider({ flag, namespace })
